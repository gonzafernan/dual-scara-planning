...
def test_forward(five):
    ...
    assert AssertionError !=np.testing.assert_allclose(five.fkine(),np.array([0.0,0.3]),rtol=1e-8,atol=1e-7)
    # -+
    five.arms[0].wkMode_ = -1
...
def test_forward_ass_2(five):
    ...
    five.arms[0].wkMode_ = -1
    five.q = np.array([1.83160416, 1.30998849])
    assert AssertionError !=np.testing.assert_allclose(five.fkine(),np.array([-1.98021962e-17, 9.61346135e-02]),rtol=1e-5,atol=1e-5)
